  ...
  return new Promise((resolve, reject) => {
    const req = https.get(url, { timeout: 15000 }, (res) => {
      if (res.statusCode === 200 || res.statusCode === 302) {
        ...
        const req2 = https.get(targetUrl, { timeout: 15000 }, (imageRes) => {
...
async function run() {
  ...
  for (let i = 1; i < productBlocks.length; i++) {
    ...
    // Skip if already successfully updated in code AND image exists
    ...
    try {
      ...
      console.log(`Success! Waiting 4 seconds before next download...`);
